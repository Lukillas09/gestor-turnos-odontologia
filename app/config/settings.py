...
TURNOS_PUBLIC_REDIS_REQUIRED = env_bool("TURNOS_PUBLIC_REDIS_REQUIRED", False)
...
TURNOS_PUBLIC_SMART_SCHEDULING_ENABLED = env_bool(
    "TURNOS_PUBLIC_SMART_SCHEDULING_ENABLED",
    False,
)
...
def env_nonnegative_int(nombre, default):
    valor = int(env(nombre, str(default)))
...
TURNOS_PUBLIC_ACCESS_REQUEST_WINDOW_SECONDS = env_nonnegative_int(
    "TURNOS_PUBLIC_ACCESS_REQUEST_WINDOW_SECONDS",
    900,
)
TURNOS_PUBLIC_OTP_ATTEMPTS = env_nonnegative_int("TURNOS_PUBLIC_OTP_ATTEMPTS", 5)
TURNOS_PUBLIC_OTP_SECONDS = env_nonnegative_int("TURNOS_PUBLIC_OTP_SECONDS", 600)
...
TURNOS_PUBLIC_BOOKING_DNI_LIMIT = env_nonnegative_int("TURNOS_PUBLIC_BOOKING_DNI_LIMIT", 5)
TURNOS_PUBLIC_BOOKING_DNI_WINDOW_SECONDS = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_DNI_WINDOW_SECONDS",
    3600,
)
TURNOS_PUBLIC_BOOKING_TURNSTILE_AFTER_ATTEMPTS = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_TURNSTILE_AFTER_ATTEMPTS",
    3,
)
TURNOS_PUBLIC_BOOKING_MAX_PENDING_PER_DNI = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_MAX_PENDING_PER_DNI",
    2,
)
TURNOS_PUBLIC_BOOKING_IDEMPOTENCY_SECONDS = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_IDEMPOTENCY_SECONDS",
    3600,
)
TURNOS_PUBLIC_BOOKING_PROCESSING_SECONDS = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_PROCESSING_SECONDS",
    120,
)
TURNOS_PUBLIC_BOOKING_DUPLICATE_WINDOW_SECONDS = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_DUPLICATE_WINDOW_SECONDS",
    86400,
)
TURNOS_PUBLIC_BOOKING_NEARBY_DAYS_LIMIT = env_nonnegative_int(
    "TURNOS_PUBLIC_BOOKING_NEARBY_DAYS_LIMIT",
    14,
)
TURNOS_PUBLIC_BOOKING_HORARIOS_CACHE_SECONDS = env_nonnegative_int(
...
TURNSTILE_REQUIRED_AFTER_ATTEMPTS = env_nonnegative_int(
    "TURNSTILE_REQUIRED_AFTER_ATTEMPTS",
    3,
)
...
_booking_hard_limits = [
    limite
    for limite in (
        TURNOS_PUBLIC_BOOKING_IP_LIMIT,
        TURNOS_PUBLIC_BOOKING_DNI_LIMIT,
    )
    if limite > 0
]
...
if _booking_hard_limits and TURNOS_PUBLIC_BOOKING_TURNSTILE_AFTER_ATTEMPTS > min(
    ...
    raise RuntimeError(
        ...
        "al menor limite duro de reservas publicas habilitado."
    )

_limites_publicos_con_ventana = (
    (
        "TURNOS_PUBLIC_ACCESS_REQUEST_LIMIT",
        TURNOS_PUBLIC_ACCESS_REQUEST_LIMIT,
        "TURNOS_PUBLIC_ACCESS_REQUEST_WINDOW_SECONDS",
        TURNOS_PUBLIC_ACCESS_REQUEST_WINDOW_SECONDS,
    ),
    (
        "TURNOS_PUBLIC_RESEND_LIMIT",
        TURNOS_PUBLIC_RESEND_LIMIT,
        "TURNOS_PUBLIC_RESEND_WINDOW_SECONDS",
        TURNOS_PUBLIC_RESEND_WINDOW_SECONDS,
    ),
    (
...
for _nombre_limite, _limite, _nombre_ventana, _ventana in _limites_publicos_con_ventana:
    if _limite > 0 and _ventana <= 0:
        raise RuntimeError(
            f"{_nombre_ventana} debe ser mayor a cero cuando {_nombre_limite} está activo."
        )

if TURNOS_PUBLIC_BOOKING_IDEMPOTENCY_SECONDS <= 0:
    raise RuntimeError("TURNOS_PUBLIC_BOOKING_IDEMPOTENCY_SECONDS debe ser mayor a cero.")

if not 0 < TURNOS_PUBLIC_BOOKING_PROCESSING_SECONDS <= TURNOS_PUBLIC_BOOKING_IDEMPOTENCY_SECONDS:
    raise RuntimeError(
        "TURNOS_PUBLIC_BOOKING_PROCESSING_SECONDS debe ser mayor a cero y menor o igual "
        "a TURNOS_PUBLIC_BOOKING_IDEMPOTENCY_SECONDS."
    )
...
GOOGLE_CALENDAR_CLIENT_SECRET_FILE = env("GOOGLE_CALENDAR_CLIENT_SECRET_FILE")
GOOGLE_CALENDAR_CLIENT_SECRETS_FILE = env(
